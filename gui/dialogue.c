...
void OnButton_send(GtkWidget *pWidget, t_window_d * window_d)
{
  ...
  gchar* StringEdit = gtk_text_buffer_get_text(pTextBuffer_edit_d,
					       &iStart, &iEnd,
					       TRUE);
  pTextBuffer_view_d = gtk_text_view_get_buffer(GTK_TEXT_VIEW(window_d->pTextView_d));
  if ( *StringEdit != '\0'){
    tmp = g_strconcat(window_d->window->me->name,":\n  ",StringEdit,"\n",'\0'); 
    ...
    switch (window_d->contact->mode)
      {
      case NETSOUL :
	
	window_d->window->me->depart->lu=0;
	window_d->window->me->depart->login = window_d->contact->login;
	window_d->window->me->depart->msg = StringEdit;
...
void OnKill_d(GtkWidget *widget, t_window_d *window_d)
{
  if ( (window_d->contact != NULL)){
    ...
    switch (window_d->contact->mode) 
      {
      case P2P : tutti_deconnect(window_d->contact->socket); 
	break;
      }
  }
  g_free(window_d);
...
void OnButton_p2p(GtkWidget *pWidget,  t_window_d * window_d)
{
  pWidget = pWidget;
  if (/*tuttiable*/ 1){
    /*window_d->contact->ip = get ip*/
    window_d->contact->socket = tutti_connect(window_d->contact->ip, 2003);
    printf("%i",window_d->contact->socket);
    window_d->contact->mode = P2P;
  }
}
